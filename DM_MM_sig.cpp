...
Type objective_function<Type>::operator() ()
{
  ...
  for(int time=0; time < nperiods; time++) {
    sig_disp(time) = exp(log_sig_disp(time));
  }
  ...
  for (int i=0; i<nperiods; i++) {
    f -= dnorm(log_sig_disp_eps(i), Type(0.0), sig_disp_sig, true);
    ...
    for (int j=0; j<nsites; j++) {
      ...
      if(disp_model == 1) { // 1 = half-normal
        ...
        // fifty_pct = qnorm(Type(0.75), Type(0.0), sig_disp(i)); // want middle of positive half of distn
        // pct_at_dist = (pnorm(dist_cutoff, Type(0.0), sig_disp(i)) - Type(0.5)) * Type(2.0);
        ...
        // fifty_pct = qexp(Type(0.5), pow(sig_disp(i), -1));
        // pct_at_dist = pexp(dist_cutoff, pow(sig_disp(i), -1));
        ...
        // fifty_pct = sig_disp(i)*tan(PI*(Type(0.75) - Type(0.5))); // want middle of positive half of distn
        // pct_at_dist = pow(PI, -1) * atan(dist_cutoff/sig_disp(i)) * Type(2.0);
      }
